...
public class EmpUpdateLogic implements CommonLogic {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) {
		if (msg.size() == 0) {
			try {
				...
				HashMap<String, String> id = empDao.getId(empId);
				argMap.put("empId", empId);
				argMap.put("addId", id.get("addId"));
				argMap.put("imgId", id.get("imgId"));

				if (empDao.updateEmp(argMap, baos.toByteArray())) {
					msg.add("社員情報を更新しました。");
					request.setAttribute("msg", msg);
					return "Result.jsp";
				} else {
					msg.add("社員情報の更新に失敗しました。");
					request.setAttribute("msg", msg);
					return "Result.jsp";
				}
			} catch (IOException e) {
	...
	public void checkWrongInput(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			argMap.put("fileName", part.getSubmittedFileName());
